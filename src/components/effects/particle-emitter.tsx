...
export default function ParticleEmitter({ config = {} }: ParticleEmitterProps) {
  ...
  useEffect(() => {
    ...
    const calcCursorPosition = (timeScale: number) => {
      ...
      const dampFactor = Math.pow(D, timeScale);
    ...
    const animate = (timestamp: number) => {
      ...
      for (let i = 0; i < emit; i++) {
        ...
        if (emitCount < particlesRef.current.length) {
          ...
          if (particle) {
            ...
            const emitX = EMath.lerp(
              cursor.prevPosition.x,
              cursor.x,
              i / Math.max(1, emit),
            );
            const emitY = EMath.lerp(
              cursor.prevPosition.y,
              cursor.y,
              i / Math.max(1, emit),
            );
            ...
            particle.target.x =
              emitX + 10 * cursorSpeed * rLength * Math.cos(direction + rAngle);
            particle.target.y =
              emitY + 10 * cursorSpeed * rLength * Math.sin(direction + rAngle);
            particle.alpha = 1;
          }
      ...
      for (const particle of particlesRef.current) {
        ...
        if (particle) {
          ...
          const speed = EMath.magnitude(
            particle.tween.velocity.x,
            particle.tween.velocity.y,
          );
          ...
          if (particle.alpha > 0.01) {
            ...
            ctx.fillStyle =
              particle.color +
              Math.floor(parseFloat(alpha) * 255)
                .toString(16)
                .padStart(2, "0");
            ctx.fill();
          }
